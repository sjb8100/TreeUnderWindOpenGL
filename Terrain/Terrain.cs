...
namespace Terrain {
	...
	public class Terrain : ILoadable, IRenderable {
		...
		private void BuildTexturemap() {
			...
			float snowHeight = minHeight + (deltaHeight * 8.0f / 10f);
            ...
            for (int x = 0; x < Texturemap.GetLength(0); x++) {
				for (int z = 0; z < Texturemap.GetLength(1); z++) {
                    ...
                    randomInfluence = 0.1f;
					bool snow = avgHeight * (1f - randomInfluence) + avgHeight * randomInfluence * RandomAt(x, z) * 2f > snowHeight;
